...
func (s *Signer) URL(base, incidentID string, openedAt time.Time, via string) string {
	...
	// ESCAPED, not concatenated. Both values are generated by this product
	// today, so nothing is broken by the old form -- but this string is a
	// CREDENTIAL that gets pasted into email, push payloads and webhook
	// bodies, and a value that later grows a "?" or a "#" would move the token
	// into a query or a fragment and quietly stop it being sent at all.
	u := fmt.Sprintf("%s/ack/%s/%s", b,
		url.PathEscape(incidentID), s.Mint(incidentID, openedAt))
	if via != "" {
