...
func channelWithCanaries(t *testing.T) (*Channel, []string) {
	t.Helper()
	const password = "SMTP-PASSWORD-CANARY-MUST-NOT-LEAK"
	ch, err := New(Config{
		Host: "smtp.example.com", Port: 587,
		Username: "alarms@example.com", Password: password,
		From: "alarms@example.com", To: []string{"ops@example.com"},
	})
	if err != nil {
		t.Fatal(err)
	}
	return ch, []string{password}
}
