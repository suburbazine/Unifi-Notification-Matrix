...
func channelWithCanaries(t *testing.T) (*Channel, []string) {
	t.Helper()
	const (
		secretVal = "WEBHOOK-SECRET-CANARY-MUST-NOT-LEAK"
		urlToken  = "URL-TOKEN-CANARY-MUST-NOT-LEAK"
	)
	// The URL is a credential too: most receivers put an unguessable token in
	// the path or the query, so printing it whole is printing a password.
	ch, err := New(Config{
		URL:    "https://receiver.example/api/webhook/" + urlToken + "?key=" + urlToken,
		Secret: secretVal,
	}, nil)
	if err != nil {
		t.Fatal(err)
	}
	return ch, []string{secretVal, urlToken}
}
